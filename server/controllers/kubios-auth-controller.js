 ...
 */
 const syncWithLocalUser = async (kubiosUser) => {
   ...
   let userId;
   const result = await userModel.selectUserByEmail(kubiosUser.email);
   ...
   if (result.error) {
     ...
     const newUserResult = await userModel.addUser(newUser);
 ...
 */
 const getMe = async (req, res) => {
   const user = await userModel.selectUserById(req.user.userId);
